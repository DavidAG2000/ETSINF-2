...
public abstract class Grafo {
    ...
    protected void dijkstra(int origen) {
        // COMPLETAR
        //inicialitzem els arrays a infinit, -1 i 0.
        int nVertex = numVertices();
        distanciaMin = new double[nVertex];
        caminoMin = new int[nVertex];
        visitados = new int[nVertex];
        ...
        ColaPrioridad<Vertex> cp = new PriorityQColaPrioridad();
        cp.insertar(new Vertex(origen, 0));
        
        while(!cp.esVacia()){
            Vertex v = cp.eliminarMin();
            int posV = v.getDestino();
            
            if(visitados[posV] == 0){
                visitados[posV] = 1;
                
                ListaConPI<Adyacente> ady = adyacentesDe(posV);
                
                for(ady.inicio(); !ady.esFin(); ady.siguiente()){
                    Adyacente a = ady.recuperar();
                    int posAdy = a.getDestino();
                    
                    if(distanciaMin[posAdy] > (distanciaMin[posV] + a.getPeso()) ){
                        distanciaMin[posAdy] = (distanciaMin[posV] + a.getPeso());
                        caminoMin[posAdy] = posV;
                        cp.insertar(new Vertex(posAdy, distanciaMin[posAdy]));
                    }
                }
            }
        }
    ...
    public ListaConPI<Integer> caminoMinimo(int origen, int destino) {      
        ...
        int v = caminoMin[destino];
        if(v == -1){ return res; }
        
        res.inicio();
        res.insertar(destino);
        ...
        return new LEGListaConPI<Integer>();
    }
}
